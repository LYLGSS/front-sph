...
export default {
  ...
  data() {
    return {
      ...
      // 支付定时器
      timer: null,
      // 支付状态码【200：成功   205：支付中】
      code: ''
    }
  },
  // 尽量别在生命周期函数中使用 async|await
  created() {
    this.getPayInfo()
  },
  computed: {
  ...
  methods: {
    ...
    async open() {
      ...
      }).catch(() => {}) // 这里 catch 为了不让控制台报错【点击“支付遇见问题”的报错】，无实际意义
      ...
      if (!this.timer) {
        ...
        this.timer = setInterval(async () => {
          const res = await this.$API.reqPayStatus(this.payInfo.orderId)
          if (res.code === 200) {
            // 1、清除定时器
            clearInterval(this.timer)
            this.timer = null
