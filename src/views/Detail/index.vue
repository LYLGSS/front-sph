    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <!-- 当数据没有回来时，skuInfo.skuImageList 是 undefined，在子组件 Zoom 中取 skuImageList[0].imgUrl 时，skuImageList 是 undefined，没有第0项，因此会报错，至少应该给子组件 Zoom 传递一个空数组，使他取第0项不会报错；但是仅仅返回一个空数组还不行，数据没有回来时，子组件中取 skuImageList[0].imgUrl 时，skuImageList[0] 是 undefined ，undefined 取 .imgUrl 还会报错，因此应该至少使空数组的第0项为一个空对象，空对象取 .imgUrl 为 undefined，不会报错-->
...
export default {
  ...
  created() {
    // 派发 action 获取产详情的信息
    this.$store.dispatch('detail/getGoodInfo', this.$route.params.goodId)
  },
  computed: {
  ...
  methods: {
    // 产品售卖属性值切换高亮
    changeActive(clickedSaleAttrValue, valueList) {
      // 参数1：当前点击的是售卖属性的值    参数2：点击的售卖属性列表
      // 遍历全部售卖属性值，将 isChecked 改为0，全部取消高亮
